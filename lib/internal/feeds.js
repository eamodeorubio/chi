...
function pluggedMethod(feed, busFactory, plugin) {
  return function () {
    return feed.chain(plugin(busFactory, slice.call(arguments)));
  };
}
...
function Feed(busFactory, stateFactory, isFeed) {
  var bus = busFactory()
      , currentState = stateFactory(bus.publish)
      , me = this;

  decorateWithPlugins(this, busFactory);
...
var self = module.exports = {
  feed:function (busFactory, stateFactory, optFeedCheck) {
    return new Feed(busFactory, stateFactory, typeof optFeedCheck === 'function' ? optFeedCheck : self.isFeed);
  },
